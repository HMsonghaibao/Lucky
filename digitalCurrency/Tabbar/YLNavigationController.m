...
+(void)initialize{
    if (self == [YLNavigationController class]) {
        ...
        //2.可以设置背景图
        bar.barTintColor =  [UIColor colorWithRed:0/255.0 green:35/255.0 blue:211/255.0 alpha:1/1.0];
        //4.可以设置标题文字的垂直位置
        ...
        attributes[NSForegroundColorAttributeName] = AppTextColor_FFFFFF;
        [bar setTitleTextAttributes:attributes];
        
        bar.tintColor = AppTextColor_FFFFFF;
        
        UIBarButtonItem *item = [UIBarButtonItem appearance];
        [item setTitleTextAttributes:@{NSForegroundColorAttributeName:AppTextColor_FFFFFF} forState:UIControlStateNormal];
        [item setTintColor:AppTextColor_FFFFFF];
...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated
{
    ...
    if (self.viewControllers.count > 0) {
        ...
        UIBarButtonItem *backBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"luckyback"] style:UIBarButtonItemStylePlain target:self action:@selector(popAction)];
...
- (void)pushViewController:(UIViewController *)viewController withBackTitle:(NSString *)backTitle action:(CommonVoidBlock)backAction animated:(BOOL)animated
{
    if (backTitle.length != 0 )
    {
        ...
        [btn setImage:[UIImage imageNamed:@"luckyback"] forState:UIControlStateNormal];
