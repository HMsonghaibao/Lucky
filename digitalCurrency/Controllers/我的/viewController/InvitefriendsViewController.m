...
@property(nonatomic,strong)UIImage *codepicImage;
@end
...
- (void)viewDidLoad {
    ...
    self.codeL.text = [NSString stringWithFormat:@"%@: %@",LocalizationKey(@"invite"),[YLUserInfo shareUserInfo].promotionCode];
...
-(void)getData{
    ...
    [MineNetManager queryInviteInfoParam:nil CompleteHandle:^(id resPonseObj, int code) {
...
//                [self.codeImage sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@%@",HOST,@"uc/member/showPromotionCodeQr?codeUrl=",resPonseObj[@"data"][@"promotion_code"]]] placeholderImage:[UIImage imageNamed:@""]];
                ...
                // 3. 将字符串转换成NSData
                NSString *urlcodeStr = resPonseObj[@"data"];//测试二维码地址,次二维码不能支付,需要配合服务器来二维码的地址(跟后台人员配合)
                NSString *urlStr = [NSString stringWithFormat:@"%@?promotionCode=%@",urlcodeStr,[YLUserInfo shareUserInfo].promotionCode];
                NSLog(@"===%@",urlStr);
                ...
                self.codepicImage = [self createNonInterpolatedUIImageFormCIImage:outputImage withSize:200];
...
//    [self.view makeToast:[[ChangeLanguage bundle] localizedStringForKey:@"copyAdressTip" value:nil table:@"English"] duration:1.5 position:CSToastPositionCenter];
    [self.view makeToast:@"复制成功" duration:1.5 position:CSToastPositionCenter];
}

- (IBAction)huodongBtnClick:(UIButton *)sender {
    //安全设置
    [self saveImage:self.codepicImage];
//    InviteDownloadViewController *accountVC = [[InviteDownloadViewController alloc] init];
