...
@property(nonatomic,copy)NSString *pasteboardbankStr;
@end
...
-(UITableView *)tableView{
    if (!_tableView) {
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, kWindowW, kWindowH-SafeAreaBottomHeight-NEW_NavHeight-self.tableBottomHeight) style:UITableViewStyleGrouped];
...
-(void)arrangeData{
    ...
    if ([_detailModel.type isEqualToString:@"0"] || [_detailModel.type isEqualToString:@"1"]) {
        //买入 填写支付信息
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        ...
        if ([self.section2ImageArr[indexPath.row] isEqualToString:@"yinlianImage"]) {
            ...
            //单击的手势
            UITapGestureRecognizer *tapRecognize = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(banknumhandleTap:)];
            tapRecognize.numberOfTapsRequired = 1;
            [tapRecognize delaysTouchesBegan];
            [tapRecognize cancelsTouchesInView];
            [cell.banknumlabel addGestureRecognizer:tapRecognize];
            self.pasteboardbankStr = cell.banknumlabel.text;
            return cell;
            ...
            [tapRecognize delaysTouchesBegan];
            [tapRecognize cancelsTouchesInView];
            [cell.rightLabel addGestureRecognizer:tapRecognize];
            self.pasteboardStr = cell.rightLabel.text;
            return cell;
        }
...
-(void) handleTap:(UITapGestureRecognizer *)recognizer
{
    ...
    pasteboard.string = self.pasteboardStr;
...
-(void) banknumhandleTap:(UITapGestureRecognizer *)recognizer
{
    UIPasteboard *pasteboard = [UIPasteboard generalPasteboard];
    pasteboard.string = self.pasteboardbankStr;
    [self.view makeToast:[[ChangeLanguage bundle] localizedStringForKey:@"copyscuuses" value:nil table:@"English"] duration:1.5 position:CSToastPositionCenter];
}
...
-(void)fangKuanAlterView{
    ...
    .LeeAddTextField(^(UITextField *textField){
        ...
        textField.layer.borderColor = RGBCOLOR(18, 22, 28).CGColor;
