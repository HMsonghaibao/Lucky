...
@interface RecommendedRewardController ()<FSPageContentViewDelegate,FSSegmentTitleViewDelegate>
@property (nonatomic, strong) UISegmentedControl *seg;
@property (nonatomic, strong) FSPageContentView * pageContentView;
@end

@implementation RecommendedRewardController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view from its nib.
    self.title = [[ChangeLanguage bundle] localizedStringForKey:@"recommendedReward" value:nil table:@"English"];
    ...
    for (NSString *title in titles) {
        if ([title isEqualToString:LocalizationKey(@"direct")]) {
            RecommendedWayController *buyVC = [[RecommendedWayController alloc] initWithChildViewType:0];
            buyVC.title = title;
            buyVC.view.backgroundColor = mainColor;
            [contentVCs addObject:buyVC];
            
        }else{
            RecommendedWayController *sellVC = [[RecommendedWayController alloc] initWithChildViewType:1];
            sellVC.title = title;
