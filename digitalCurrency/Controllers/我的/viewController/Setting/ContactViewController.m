...
- (void)initUI{
    ...
    UILabel *bccoin = [[UILabel alloc] initWithFrame:CGRectMake(15, 35, kWindowW - 30, 20)];
    bccoin.textColor = RGBOF(0xe6e6e6);
    bccoin.font = [UIFont systemFontOfSize:17];
    bccoin.text = LocalizationKey(@"aboutbccoin");
    [self.scrollView addSubview:bccoin];
    ...
    about.frame = CGRectMake(15, CGRectGetMaxY(bccoin.frame) + 30, kWindowW - 30, height);
    ...
    idea.text = LocalizationKey(@"bccoinIdea");
    ...
    target.text = LocalizationKey(@"bccoinTarget");
...
//    kefuLabel.text = @"NBTC邮箱：yuanheng@yhliz.com";
    kefuLabel.text = [NSString stringWithFormat:@"NTEX%@：business@nbtcex.com",LocalizationKey(@"email")];
    kefuLabel.hidden = YES;
//    kefuLabel.textAlignment = NSTextAlignmentCenter;
    [self.scrollView addSubview:kefuLabel];
    
    UIImageView *qun = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"info-huite"]];
    qun.frame = CGRectMake(0, 0, 72.5 * kWindowWHOne, 72.5 * kWindowWHOne);
    qun.center = CGPointMake(kWindowW / 2, CGRectGetMaxY(kefuLabel.frame) + 25 + (72.5 * kWindowWHOne) / 2);
