...
@interface PartnerSubViewController ()<UITableViewDataSource, UITableViewDelegate,SelectCoinTypeView1Delegate>{
...
@implementation PartnerSubViewController
...
//    self.title = [[ChangeLanguage bundle] localizedStringForKey:@"Partnerapplication" value:nil table:@"English"];
    
    if ([self.title isEqualToString:LocalizationKey(@"Upgradepartnerttt")]) {
        self.titleLabel.text = LocalizationKey(@"Upgradepartner");//@"升级合伙人~";
    }else{
        self.titleLabel.text = LocalizationKey(@"Applytobecomeapartner"); //@"申请成为合伙人~";
    }
...
- (void)UILayout{
//    self.titleLabel.text = [NSString stringWithFormat:@"%@邀请你成为项目合伙人~",self.nameFrom];
    ...
    _codetf.placeholder = LocalizationKey(@"CodePlacejolder");
    _payTypeLabel.text = LocalizationKey(@"SelectPaymentMethod");
    _alterLabel.text = LocalizationKey(@"ApplyAlartString");
    [_confirmButton setTitle:LocalizationKey(@"ok") forState:UIControlStateNormal];
}
...
//    }];
    ...
    if ([NSString stringIsNull:self.coinId]) {
        [self.view makeToast:LocalizationKey(@"Pleaseenterthecurrencyname") duration:1.5 position:CSToastPositionCenter];
        return;
    }
    
    //    if ([NSString stringIsNull:self.codeTF.text]) {
    //        [self makeToast:LocalizationKey(@"inputinvite") duration:1.5 position:CSToastPositionCenter];
    //        return;
    //    }
    
    
    [EasyShowLodingView showLodingText:[[ChangeLanguage bundle] localizedStringForKey:@"loading" value:nil table:@"English"]];
    [LoginNetManager partnerapplicationForPhone:@"" withcoinName:@"" withcoin:self.coinId withcode:self.codetf.text withusername:@"" withapplyType:[NSString stringWithFormat:@"%lu",self.applyType] withdesc:@"" CompleteHandle:^(id resPonseObj, int code) {
        [EasyShowLodingView hidenLoding];
        NSLog(@"=====%@",resPonseObj);
        if (code) {
            if ([resPonseObj[@"code"] integerValue]==0) {
    ...
    [MineNetManager getApplyMoneyParam:bodydic CompleteHandle:^(id resPonseObj, int code) {
        ...
        if (code) {
            if ([resPonseObj[@"code"] integerValue]==0) {
                ...
                self.applyType = [model0.partnerType integerValue];
                if (self.applyType == 1) {
                    self.typeLabel.text = LocalizationKey(@"BusinessPartner");
                }else if (self.applyType == 2){
                    self.typeLabel.text = LocalizationKey(@"ProjectPartner");
                }
...
- (void)selectCoinTypeModel:(applyPartnerModel *)model enterIndex:(NSInteger)index payWaysArr:(NSMutableArray *)payWaysArr {
    ...
    self.priceLabel.text = [NSString stringWithFormat:@"支付金额：%@", str];
