...
@interface TeamManagerViewController () <FSPageContentViewDelegate,FSSegmentTitleViewDelegate>
...
//    self.title = [[ChangeLanguage bundle] localizedStringForKey:@"Teammanagement" value:nil table:@"English"];
    ...
    NSArray *titles = @[@"活期账单",@"定期账单",@"推广奖励"];
    NSMutableArray *contentVCs = [NSMutableArray array];
    for (NSString *title in titles) {
        if ([title isEqualToString:@"活期账单"]) {
            RecommendedWayController *buyVC = [[RecommendedWayController alloc] initWithChildViewType:1];
            buyVC.title = title;
...
- (UISegmentedControl *)seg{
    if (!_seg) {
        _seg = [[UISegmentedControl alloc] initWithItems:@[@"活期账单",@"定期账单",@"推广奖励"]];
