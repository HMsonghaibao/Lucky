...
@interface LuckyTranViewController ()<UITextFieldDelegate>
...
@property (nonatomic, copy) NSString * fensu;
@end

@implementation LuckyTranViewController
...
//    self.loginPassword.placeholder = [[ChangeLanguage bundle] localizedStringForKey:@"inputLoginPassword" value:nil table:@"English"];
    ...
    self.latestPhoneNum.delegate = self;
    [self requestData];
}
...
- (void)textFieldDidEndEditing:(UITextField *)textField{
    ...
    self.feeL.text = [NSString stringWithFormat:@"%@: %.2fUSDT",LocalizationKey(@"Availablequantityttttt"),self.latestPhoneNum.text.doubleValue*(1-self.fensu.doubleValue)];
    ...
    [MineNetManager getLotteryRule:bodydic CompleteHandle:^(id resPonseObj, int code) {
        ...
        if (code) {
            if ([resPonseObj[@"code"] integerValue]==0) {
                //获取数据成功
                self.numL.text = [NSString stringWithFormat:@"%@: %.2fUSDT",LocalizationKey(@"Availablequantityttt"),[resPonseObj[@"data"][@"balance"] doubleValue]];
                self.fensu = [NSString stringWithFormat:@"%@",resPonseObj[@"data"][@"transferRatio"]];
...
- (IBAction)btnBlick:(UIButton *)sender {
    ...
    [bodydic setValue:self.codeTextField.text forKey:@"targetUserName"];
