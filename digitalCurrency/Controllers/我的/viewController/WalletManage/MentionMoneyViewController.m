...
-(void)arrageData{
    for (MentionCoinInfoModel *mentionModel in self.mentionCoinArr) {
        ...
        if ([mentionModel.unit isEqualToString:self.unit]) {
            ...
            self.numTextField.placeholder = [NSString stringWithFormat:@"%@%@",[[ChangeLanguage bundle] localizedStringForKey:@"Minimumnumberofcoins" value:nil table:@"English"],[ToolUtil formartScientificNotationWithString:self.model.minAmount]];
    ...
    [MineNetManager mentionCoinApplyForUnit:self.model.unit withAddress:self.mentionMoneyAddress.text withAmount:self.numTextField.text withFee:self.poundageNum.text withRemark:remark withJyPassword:self.moneyPassword.text mobilecode:self.mobilecodeTF.text  CompleteHandle:^(id resPonseObj, int code) {
        ...
        if (code) {
            if ([resPonseObj[@"code"] integerValue] == 0) {
                ...
                NSMutableArray*array= [addressInfoArr mutableCopy];
                NSDictionary*dict = @{@"remark":self.model.unit,@"address":self.mentionMoneyAddress.text};
                if (![array containsObject:dict]) {
                    [array addObject:dict];
                }
                [[NSUserDefaults standardUserDefaults] setObject:array forKey:@"addressInfoArr"];
