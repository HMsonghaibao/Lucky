...
@property (nonatomic,strong)NSMutableArray *investarr;
@end
...
-(void)rightButton{
    ...
    [issueButton setTitle:LocalizationKey(@"Englishttt") forState:0];
...
- (void)leftItem{
    UIButton * issueButton = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    issueButton.frame = CGRectMake(0, 0, 25, 25);
    [issueButton setTitle:LocalizationKey(@"lucky") forState:0];
    issueButton.titleLabel.font = [UIFont systemFontOfSize:18];
    //添加到导航条
    UIBarButtonItem *leftBarButtomItem = [[UIBarButtonItem alloc]initWithCustomView:issueButton];
...
//    }
    ...
    LanguageSettingsViewController *groupVC = [[LanguageSettingsViewController alloc] init];
...
{
    if (indexPath.section==0) {
        MyrewardTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MyrewardTableViewCell"];
        ...
        cell.selectBlock = ^(NSInteger num) {
            ...
            //申请投资
            InvestmentSubViewController *detailVC = [[InvestmentSubViewController alloc] initWithChildViewType:num];
            detailVC.hidesBottomBarWhenPushed = YES;
            detailVC.model = self.investarr[num];
            detailVC.title = LocalizationKey(@"Applyforinvestment");
        ...
        MyinvestmentTableViewCell * cell = [tableView dequeueReusableCellWithIdentifier:@"MyinvestmentTableViewCell" forIndexPath:indexPath];
        cell.selectionStyle=UITableViewCellSelectionStyleNone;
        ...
        cell.selectBlock = ^(NSInteger num) {
            if(![YLUserInfo isLogIn]){
                ...
                //申请投资
                [self.view makeToast:LocalizationKey(@"Developingsostaytuned") duration:1.5 position:CSToastPositionCenter];
            }
...
//    [EasyShowLodingView showLodingText:[[ChangeLanguage bundle] localizedStringForKey:@"loading" value:nil table:@"English"]];
    ...
    [MineNetManager getInviteMemberListParam:nil CompleteHandle:^(id resPonseObj, int code) {
        [EasyShowLodingView hidenLoding];
        ...
        if (code) {
            if ([resPonseObj[@"code"] integerValue]==0) {
                //获取数据成功
                [_investarr removeAllObjects];

                NSArray *dataArr = [investModel mj_objectArrayWithKeyValuesArray:resPonseObj[@"data"]];
                [self.investarr addObjectsFromArray:dataArr];
                [self.tableView reloadData];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    if (section==0) {
        if (self.platformMessageArr.count>0) {
            ...
            CustomSectionHeader*sectionView=[CustomSectionHeader instancesectionHeaderViewWithFrame:CGRectMake(0, 0, kWindowW, 40)];
            pageScrollView *noticeView = [[pageScrollView alloc] initWithFrame:CGRectMake(78, 0, kWindowW-110, 35)];
            ...
            [sectionView.backView addSubview:noticeView];
