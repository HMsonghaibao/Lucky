...
@interface MyLuckyIncomeViewController ()<UITableViewDataSource, UITableViewDelegate>
...
@implementation MyLuckyIncomeViewController
    ...
    [MineNetManager getInvestRewardList:bodydic CompleteHandle:^(id resPonseObj, int code) {
        ...
        if (code) {
            if ([resPonseObj[@"code"] integerValue]==0) {
                //获取数据成功
                self.num1L.text = [NSString stringWithFormat:@"%@",resPonseObj[@"data"][@"incomeTotal"]];
                self.myredLabel.text = [NSString stringWithFormat:@"%@",resPonseObj[@"data"][@"haveGotIncomeTotal"]];
                self.num2L.text = [NSString stringWithFormat:@"%@",resPonseObj[@"data"][@"surplusTotal"]];
    ...
    [MineNetManager getInvestRewardTeamList:bodydic CompleteHandle:^(id resPonseObj, int code) {
        ...
        if (code) {
            if ([resPonseObj[@"code"] integerValue]==0) {
                ...
                NSArray *array = [RedPacketModel mj_objectArrayWithKeyValuesArray:resPonseObj[@"data"][@"rows"]];
...
- (void)UILayout{
    ...
    self.title = LocalizationKey(@"income");
    self.amountL.text = LocalizationKey(@"Amountttt");
    self.dateL.text = LocalizationKey(@"Datettt");
    self.typeL.text = LocalizationKey(@"typettt");
    self.dailingLabel.text = LocalizationKey(@"Gainableincomettt");
    self.yilingLabel.text = LocalizationKey(@"Earnedgainsttt");
    self.amountLabel.text = LocalizationKey(@"Residualincomettt");
