...
- (IBAction)currencyaction:(id)sender {
    ...
    [YBPopupMenu showAtPoint:relyPoint titles:self.titlearrays icons:nil menuWidth:80 otherSettings:^(YBPopupMenu *popupMenu) {
        ...
        popupMenu.backColor = RGBCOLOR(18, 22, 28);
    ...
    [manager POST:urlString parameters:dic constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        if ([responseObject[@"code"] integerValue] == 0) {
            ...
            if (self.ispreson) {
                ...
                [self.presonProvebutton setTitle:@"已上传" forState:UIControlStateNormal];
                ...
                [self.NumProvebutton setTitle:@"已上传" forState:UIControlStateNormal];
...
- (IBAction)ApplyNowaction:(id)sender {
    ...
    if ([NSString stringIsNull:self.wechatTF.text]) {
        [self.view makeToast:@"请输入微信号" duration:1.5 position:CSToastPositionCenter];
        return;
    }
  
    if ([NSString stringIsNull:self.QQTF.text]) {
        [self.view makeToast:@"请输入QQ号" duration:1.5 position:CSToastPositionCenter];
        return;
    }
    
    if ([NSString stringIsNull:self.presonProveurl]) {
        [self.view makeToast:@"请上传个人数字资产证明" duration:1.5 position:CSToastPositionCenter];
        return;
    }
    if ([NSString stringIsNull:self.presonProveurl]) {
        [self.view makeToast:@"请上传数字交易资产证明" duration:1.5 position:CSToastPositionCenter];
        return;
    }
...
//    [bodydic setValue:self.BusModel.amount  forKey: @"amount"];
    [bodydic setValue:@"ETH" forKey: @"coinSymbol"];
    [bodydic setValue:@"3"  forKey: @"amount"];
//    [bodydic setValue:self.QQTF.text forKey:@"qq"];
...
//    NSDictionary *jsondic = @{@"json" : [self dictionaryToJson:bodydic],@"businessAuthDepositId" : self.BusModel.ID};
    NSDictionary *jsondic = @{@"json" : [self dictionaryToJson:bodydic],@"businessAuthDepositId" : @"1"};
    [EasyShowLodingView showLodingText:[[ChangeLanguage bundle] localizedStringForKey:@"loading" value:nil table:@"English"]];
