...
- (void)setCoinUserInfoModel:(CoinUserInfoModel *)coinUserInfoModel{
    ...
    if ([coinUserInfoModel.advertiseType isEqualToString:@"1"]) {
    ...
    self.priceLabel.text = [NSString stringWithFormat:@"%@CNY",_coinUserInfoModel.price];
...
-(void)cellForArray:(NSArray *)payArr{
    if (payArr.count == 1) {
        if ([payArr[0] isEqualToString:@"支付宝"]) {
            self.payImageView1.hidden = NO;
            self.payImageView2.hidden = YES;
            self.payImageView3.hidden = YES;
            ...
            self.payImageView1.hidden = YES;
            self.payImageView2.hidden = NO;
            self.payImageView3.hidden = YES;
