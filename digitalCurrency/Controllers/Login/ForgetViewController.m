...
//     self.verifyButton.delegate = self;
    ...
    self.userNameL.text = LocalizationKey(@"username");
    ...
    if (kWindowW==320) {
        ...
        if (kWindowH==812) {
            self.heightConstraint.constant = 430;
        }else{
            self.heightConstraint.constant = 380;
        }
...
- (IBAction)confirm:(UIButton *)sender {
    ...
    [EasyShowLodingView showLodingText:LocalizationKey(@"loading")];
    [LoginNetManager FogetToResetPasswordWithaccount:self.phoneNumberTF.text withname:self.userNameTF.text withcode:self.verifyCodeTF.text withMode:mode Withpassword:self.passWordTF.text CompleteHandle:^(id resPonseObj, int code) {
        [EasyShowLodingView hidenLoding];
        if (code) {
