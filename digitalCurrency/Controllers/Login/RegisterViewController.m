...
@property (weak, nonatomic) IBOutlet UILabel *RecommendationcodeL;
@property (weak, nonatomic) IBOutlet UITextField *RecommendationcodeTF;
...
//    [self rightBarItemWithTitle:LocalizationKey(@"emailRegister")];
    ...
    [_RecommendationcodeTF setValue:RGBOF(0x999999) forKeyPath:@"_placeholderLabel.textColor"];
    ...
    [_RecommendationcodeTF setValue:[UIFont boldSystemFontOfSize:12] forKeyPath:@"_placeholderLabel.font"];
...
//    self.tfHeight.constant = 30 * kWindowWHOne;
    ...
    if (kWindowW==320) {
        ...
        self.tfHeight.constant = 20;
    }else if (kWindowW==375){
        if (kWindowH==812) {
            self.heightConstraint.constant = 340;
            self.tfHeight.constant = 30;
        }else{
            self.heightConstraint.constant = 300;
            self.tfHeight.constant = 25;
        }
    ...
    [self.readBtn setTitle:LocalizationKey(@"readandagree") forState:UIControlStateNormal];
    ...
    [self.countryBtn setTitle:LocalizationKey(@"Pleaseselectacountry") forState:0];
    self.RecommendationcodeL.text = LocalizationKey(@"Recommendationcode");
    self.RecommendationcodeTF.placeholder = [NSString stringWithFormat:@"%@",LocalizationKey(@"Recommendationcode")];
...
//    detailVC.content = self.contentstr;
    detailVC.type = @"2";
    detailVC.content = @"http://h5.lucky.emei.io/protocol.html";
...
//        self.widthDistance.constant=[self calculateRowWidth:model.zhName]+5;
        if ([[ChangeLanguage userLanguage] isEqualToString:@"en"]) {
            [weakSelf.countryBtn setTitle:model.enName forState:0];
            
        }else if ([[ChangeLanguage userLanguage] isEqualToString:@"zh-Hans"])
        {
            [weakSelf.countryBtn setTitle:model.zhName forState:0];
        }
...
//            if (![self.countryBtn.titleLabel.text isEqualToString:LocalizationKey(@"Pleaseselectacountry")]) {
//                [self getphonncode];
//                return NO;
//            }else{
//                [self.view makeToast:LocalizationKey(@"Pleaseselectacountry") duration:1.5 position:CSToastPositionCenter];
//            }
            [self getphonncode];
            return NO;
//            return YES;
...
//            [postResult appendFormat:@"%@", [NSString stringWithFormat:@"mobile=%@&country=%@&ticket=%@&randstr=%@",self.phoneNumberTF.text,LocalizationKey(@"china"),resultJSON[@"ticket"],resultJSON[@"randstr"]]];
            
            [postResult appendFormat:@"%@", [NSString stringWithFormat:@"mobile=%@&codeType=%d",self.phoneNumberTF.text,0]];
            
            NSDictionary *headerFields = @{@"Content-Type":@"application/x-www-form-urlencoded;charset=UTF-8"};
            NSMutableURLRequest *secondaryRequest = [NSMutableURLRequest requestWithURL:[NSURL URLWithString: [NSString stringWithFormat:@"%@%@",HOST,@"sms/sendCheckCode"]] cachePolicy:NSURLRequestReloadIgnoringCacheData timeoutInterval:15.0];
            secondaryRequest.HTTPMethod = @"POST";
            secondaryRequest.allHTTPHeaderFields = headerFields;
            
            
            NSDictionary*dict = @{@"mobile":self.phoneNumberTF.text,@"codeType":@"0"};
...
- (void)goRegister{
    ...
    NSLog(@"注册返回信息====%@",self.countryBtn.titleLabel.text);
...
//        }
    ...
    if ([NSString stringIsNull:self.RecommendationcodeTF.text]) {
        [self.view makeToast:LocalizationKey(@"inputinvite") duration:1.5 position:CSToastPositionCenter];
        return;
    }
    ...
    [LoginNetManager getAccountForPhone:self.phoneNumberTF.text withpassword:[MD5 md5:self.passwordTF.text] withcode:self.verifyCodeTF.text withusername:self.userNameTF.text withrecommend:self.RecommendationcodeTF.text withcountry:LocalizationKey(@"china") withG3Result:codeDic CompleteHandle:^(id resPonseObj, int code) {
        
        [EasyShowLodingView hidenLoding];
        NSLog(@"注册返回信息====%@",resPonseObj);
        if (code) {
            if ([resPonseObj[@"code"] integerValue] == 0) {
