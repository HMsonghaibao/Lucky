...
@interface ForgetView1Controller ()<CaptchaButtonDelegate>
{
    BOOL _isEmail;//邮箱认证
}
...
@implementation ForgetView1Controller

- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title=LocalizationKey(@"Modifytheloginpassword");
...
//            }];
            [postResult appendFormat:@"%@", [NSString stringWithFormat:@"mobile=%@&codeType=%d",self.phoneNumberTF.text,1]];
            ...
            NSDictionary*dict = @{@"mobile":self.phoneNumberTF.text,@"codeType":@"1"};
...
- (IBAction)confirm:(UIButton *)sender {
    ...
    [EasyShowLodingView showLodingText:LocalizationKey(@"loading")];
    [LoginNetManager ChangeToResetPasswordWithaccount:self.phoneNumberTF.text withcode:self.verifyCodeTF.text withMode:mode Withpassword:self.passWordTF.text CompleteHandle:^(id resPonseObj, int code) {
        [EasyShowLodingView hidenLoding];
        if (code) {
