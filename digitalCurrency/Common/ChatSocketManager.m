...
static  NSString * s_host = @"39.98.78.108";//测试
//static  NSString * s_host = @"47.244.184.37";//生产
...
+ (instancetype)share
{
    ...
    static ChatSocketManager *instance = nil;
...
//发送消息
- (void)ChatsendMsgWithLength:(int)length withsequenceId:(long)sequenceId withcmd:(short)cmd withVersion:(int)Version withRequestId:(int)RequestId withbody:(NSDictionary*)jsonDict{
...
-(void)longConnectToSocket{
    [[ChatSocketManager share] ChatsendMsgWithLength:SOCKETREQUEST_LENGTH withsequenceId:0 withcmd:HEARTBEAT withVersion:COMMANDS_VERSION withRequestId: 0 withbody:nil];
}
...
//                [_delegate ChatdelegateSocket:sock didReadData:dataOne withTag:tag];
//            }
            [_delegate ChatdelegateSocket:sock didReadData:dataOne withTag:tag];
...
- (void)socketDidDisconnect:(GCDAsyncSocket *)sock withError:(NSError *)err{
    NSLog(@"聊天类socket断开连接--出错--%@",[err description]);
    //[APPLICATION.window makeToast:@"聊天类socket断开连接" duration:5.5 position:CSToastPositionCenter];
